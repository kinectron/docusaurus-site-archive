...
function preload() {
  beach = loadImage("./assets/images/beach.png");
}


function setup() {
  let canvas = createCanvas(640, 426);
  ...
  if (!liveData) {
    textSize(18);
    text(`
    Loading... give it a good 10 seconds... 
    (but this may not work if you have adblock installed)`, 50, 50);

    // turn recorded kinectron video into images so they are "served" like kinectron feed
    videoToImages('./assets/images/vid_black.webm')
